...
console.log(`[CONFIG] MongoDB URI: ${process.env.MONGO_URI ? "***" : "mongodb://localhost:27017/olaCris (par défaut)"}`);
console.log(`[CONFIG] Google Client ID: ${process.env.GOOGLE_CLIENT_ID ? "***" : "Non défini"}`);
...
mongoose.connect(MONGO_URI)
  ...
  .catch((err) => console.error("[MONGO] Échec de la connexion :", err.message));
...
userSchema.methods.matchPassword = async function (enteredPassword) {
  if (!this.password) {
    console.log(`[AUTH] Tentative de connexion sans mot de passe (Google ID: ${this.googleId})`);
    return false;
  }
...
const authenticateToken = (req, res, next) => {
  ...
  console.log(`[JWT] Tentative d'authentification avec token: ${token ? "***" : "Aucun"}`);

  if (!token) return res.status(401).json({ message: "Accès non autorisé : Aucun jeton fourni." });
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    ...
    if (!user.password && user.googleId) {
      console.log(`[LOGIN] Erreur: Utilisateur ${email} doit utiliser Google Auth`);
      return res.status(401).json({ message: "Veuillez vous connecter avec Google." });
    }
...
app.post("/api/auth/google", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      if (!user.googleId) {
        console.log(`[GOOGLE] Liaison du compte Google à l'utilisateur existant ${email}`);
        user.googleId = googleId;
        user.picture = picture;
        await user.save();
      } else if (user.googleId !== googleId) {
        console.log(`[GOOGLE] Conflit: Email ${email} déjà lié à un autre Google ID`);
...
app.get("/api/protected-resource", authenticateToken, (req, res) => {
  ...
  res.json({
    message: `Bienvenue ${req.user.firstName || req.user.email}, ceci est une ressource protégée !`,
    user: req.user,
  });
});
